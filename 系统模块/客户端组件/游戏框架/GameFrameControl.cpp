...
#define USER_VIEW_CY				218									//用户视图
#define MEDAL_VIEW_CY				0//36									//奖牌视图
...
VOID CGameFrameControl::RectifyControl(INT nWidth, INT nHeight)
{
	...
	INT nYPosChat=rcChatArea.bottom-58;
	CSkinButton * ChatButtonArray[]={&m_btChatButton1,&m_btChatButton2,&m_btChatButton3,&m_btChatButton4};

	//聊天控件
	DeferWindowPos(hDwp,m_btSendChat,NULL,rcChatArea.right-43,rcChatArea.bottom-27,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btChatShort,NULL,rcChatArea.right-66,rcChatArea.bottom-27,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_ChatEdit,NULL,rcChatArea.left+10,rcChatArea.bottom-19,rcChatArea.Width()-80,12,uFlags);
	if (!m_bEnableChat)
	{
		m_btSendChat.EnableWindow(false);
		m_btChatShort.EnableWindow(false);
