...
//广告偏移
#define	BILL_T						140									//广告上高
#define	BILL_L						7									//广告左宽
#define	BILL_R						7									//广告右宽
#define	BILL_B						5									//广告下高
...
VOID CGameFrameUserView::RectifyControl(INT nWidth, INT nHeight)
{
	...
	//调整按钮
	m_btUserMedal.SetWindowPos(NULL,nXBorad+nXButtonPos,nHeight-rcButton.Height()-88,0,0,SWP_NOZORDER|SWP_NOSIZE);
	m_btLoveLiness.SetWindowPos(NULL,nXBorad+nXButtonPos*2+rcButton.Width(),nHeight-rcButton.Height()-88,0,0,SWP_NOZORDER|SWP_NOSIZE);
...
BOOL CGameFrameUserView::OnEraseBkgnd(CDC * pDC)
{
	...
	if (m_pICurrentUserItem!=NULL)
	{
		...
		{
			pFaceItemControl->DrawFaceNormal(pBufferDC,18,nYExcursion+11,pCustomFaceInfo->dwCustomFace);
		}
		else
		{
			pFaceItemControl->DrawFaceNormal(pBufferDC,18,nYExcursion+11,m_pICurrentUserItem->GetFaceID());
		}
		...
		//游戏标识
		_sntprintf(szString,CountArray(szString),TEXT("奖牌：%ld"),pUserInfo->dwUserMedal);
...
INT CGameFrameUserView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_btUserMedal.SetButtonImage(IDB_BT_USER_DEMAL,hInstance,false,false);
	m_btLoveLiness.SetButtonImage(IDB_BT_LOVE_LINESS,hInstance,false,false);
