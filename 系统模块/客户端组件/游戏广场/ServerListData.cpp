...
//设置接口
VOID CServerListData::SetServerListDataSink(IServerListDataSink * pIServerListDataSink, IServerListDataSink *pIPlazaViewItemSink)
{
	//设置变量
	m_pIServerListDataSink=pIServerListDataSink;
	m_pIPlazaViewItemSink=pIPlazaViewItemSink;
...
VOID CServerListData::OnEventListFinish()
{
	...
	//事件通知
	ASSERT(m_pIPlazaViewItemSink!=NULL);
	if (m_pIPlazaViewItemSink!=NULL) m_pIPlazaViewItemSink->OnGameItemFinish();
...
VOID CServerListData::OnEventKindFinish(WORD wKindID)
{
	...
	//事件通知
	ASSERT(m_pIPlazaViewItemSink!=NULL);
	if (m_pIPlazaViewItemSink!=NULL) m_pIPlazaViewItemSink->OnGameKindFinish(wKindID);
...
VOID CServerListData::OnEventDownLoadFinish(WORD wKindID)
{
	...
	if (pGameKindItem!=NULL)
	{
		...
		//ASSERT(m_pIPlazaViewItemSink!=NULL);
		//if (m_pIPlazaViewItemSink!=NULL) m_pIPlazaViewItemSink->OnGameItemUpdate(pGameKindItem);
	}
...
VOID CServerListData::SetServerOnLineCount(WORD wServerID, DWORD dwOnLineCount)
{
	...
	if (pGameServerItem!=NULL)
	{
		...
		//ASSERT(m_pIPlazaViewItemSink!=NULL);
		//if (m_pIPlazaViewItemSink!=NULL) m_pIPlazaViewItemSink->OnGameItemUpdate(pGameServerItem);
...
VOID CServerListData::SetServerOnLineFinish()
{
	...
	ASSERT(m_pIPlazaViewItemSink!=NULL);
	if (m_pIPlazaViewItemSink!=NULL) m_pIPlazaViewItemSink->OnGameItemUpdateFinish();
}
...
bool CServerListData::InsertGameType(tagGameType * pGameType)
{
	...
	if (bInsert==true)
	{
		...
		m_pIPlazaViewItemSink->OnGameItemInsert(pGameTypeItem);
	}
	else
	{
		//界面更新
		m_pIServerListDataSink->OnGameItemUpdate(pGameTypeItem);
		//m_pIPlazaViewItemSink->OnGameItemUpdate(pGameTypeItem);
	}
...
bool CServerListData::InsertGameKind(tagGameKind * pGameKind)
{
	...
	if (bInsert==true)
	{
		...
		m_pIPlazaViewItemSink->OnGameItemInsert(pGameKindItem);
	}
	else
	{
		//更新子项
		m_pIServerListDataSink->OnGameItemUpdate(pGameKindItem);
		//m_pIPlazaViewItemSink->OnGameItemUpdate(pGameKindItem);
	}
...
bool CServerListData::InsertGameNode(tagGameNode * pGameNode)
{
	...
	if (bInsert==true)
	{
		...
		m_pIPlazaViewItemSink->OnGameItemInsert(pGameNodeItem);
	}
	else 
	{
		//更新子项
		m_pIServerListDataSink->OnGameItemUpdate(pGameNodeItem);
		//m_pIPlazaViewItemSink->OnGameItemUpdate(pGameNodeItem);
	}
...
bool CServerListData::InsertGamePage(tagGamePage * pGamePage)
{
	...
	if (bInsert==true)
	{
		...
		m_pIPlazaViewItemSink->OnGameItemInsert(pGamePageItem);
	}
	else 
	{
		//更新子项
		m_pIServerListDataSink->OnGameItemUpdate(pGamePageItem);
		//m_pIPlazaViewItemSink->OnGameItemUpdate(pGamePageItem);
	}
...
bool CServerListData::InsertGameServer(tagGameServer * pGameServer)
{
	...
	if(bInsert==true && pGameServerItem->m_pGameKindItem)
	{
		...
		//ASSERT(m_pIPlazaViewItemSink!=NULL);
		//if (m_pIPlazaViewItemSink!=NULL) m_pIPlazaViewItemSink->OnGameItemUpdate(pGameServerItem->m_pGameKindItem);
	}
	...
	if (bInsert==true)
	{
		...
		m_pIPlazaViewItemSink->OnGameItemInsert(pGameServerItem);
	}
	else 
	{
		//更新子项
		m_pIServerListDataSink->OnGameItemUpdate(pGameServerItem);
		//m_pIPlazaViewItemSink->OnGameItemUpdate(pGameServerItem);
	}
...
bool CServerListData::DeleteGameType(WORD wTypeID)
{
	...
	ASSERT(m_pIPlazaViewItemSink!=NULL);
	if (m_pIPlazaViewItemSink!=NULL) m_pIPlazaViewItemSink->OnGameItemDelete(pGameTypeItem);
...
bool CServerListData::DeleteGameKind(WORD wKindID)
{
	...
	ASSERT(m_pIPlazaViewItemSink!=NULL);
	if (m_pIPlazaViewItemSink!=NULL) m_pIPlazaViewItemSink->OnGameItemDelete(pGameKindItem);
...
bool CServerListData::DeleteGameNode(WORD wNodeID)
{
	...
	ASSERT(m_pIPlazaViewItemSink!=NULL);
	if (m_pIPlazaViewItemSink!=NULL) m_pIPlazaViewItemSink->OnGameItemDelete(pGameNodeItem);
...
bool CServerListData::DeleteGameServer(WORD wServerID)
{
	...
	ASSERT(m_pIPlazaViewItemSink!=NULL);
	if (m_pIPlazaViewItemSink!=NULL) m_pIPlazaViewItemSink->OnGameItemDelete(pGameServerItem);
...
bool CServerListData::DeleteGamePage(WORD wPageID)
{
	...
	ASSERT(m_pIPlazaViewItemSink!=NULL);
	if (m_pIPlazaViewItemSink!=NULL) m_pIPlazaViewItemSink->OnGameItemDelete(pGamePageItem);
