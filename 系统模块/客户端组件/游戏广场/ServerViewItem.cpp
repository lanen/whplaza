...
//常量定义
#define PROPERTY_CY					84									//游戏道具
#define PUBLICIZE_CY				80									//游戏广告
#define INCISE_SCALE				58/100								//界面分割
...
#define IDC_OTHER_SERVER			105									//按钮控件
...
BEGIN_MESSAGE_MAP(CServerViewItem, CDialog)
	...
	//按钮消息
	ON_BN_CLICKED(IDC_OTHER_SERVER, OnClickedOtherServer)
...
CServerViewItem::CServerViewItem()
{
	...
	//用户资源
	tagEncircleResource	EncircleAD;
	EncircleAD.pszImageTL=MAKEINTRESOURCE(IDB_SERVER_AD_TL);
	EncircleAD.pszImageTM=MAKEINTRESOURCE(IDB_SERVER_AD_TM);
	EncircleAD.pszImageTR=MAKEINTRESOURCE(IDB_SERVER_AD_TR);
	EncircleAD.pszImageML=MAKEINTRESOURCE(IDB_SERVER_AD_ML);
	EncircleAD.pszImageMR=MAKEINTRESOURCE(IDB_SERVER_AD_MR);
	EncircleAD.pszImageBL=MAKEINTRESOURCE(IDB_SERVER_AD_BL);
	EncircleAD.pszImageBM=MAKEINTRESOURCE(IDB_SERVER_AD_BM);
	EncircleAD.pszImageBR=MAKEINTRESOURCE(IDB_SERVER_AD_BR);
	m_ADEncircle.InitEncircleResource(EncircleAD,AfxGetInstanceHandle());
...
BOOL CServerViewItem::OnInitDialog()
{
	...
	m_btOtherServer.Create(NULL,WS_CHILD|WS_VISIBLE,rcCreate,this,IDC_OTHER_SERVER);
	...
	m_ChatEdit.SetEnableColor(RGB(255,255,255),RGB(50,54,67),RGB(50,54,67));
	if (!m_bEnableChat)
	{
	...
	m_ChatMessage.SetExpressionManager(CExpressionManager::GetInstance(),RGB(38,41,52));
	m_ChatMessage.SetBackgroundColor(false,RGB(38,41,52));
	...
	m_btOtherServer.SetButtonImage(IDB_BT_OTHER_SERVER,hInstance,true,false);
	...
	if (!m_bEnableChat)
	{
		m_btSendChat.EnableWindow(false);
		m_btChatShort.EnableWindow(false);
...
BOOL CServerViewItem::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	case IDM_SERVER_VIEW_ITEM_NULL:
		{
			//ASSERT(CWndViewItemCtrl::GetInstance()!=NULL);
	...
	//
	if(nCommandID >= IDM_SERVER_VIEW_ITEM_BEGIN && nCommandID <= IDM_SERVER_VIEW_ITEM_END)
	{
		//变量定义
		int nIndex = nCommandID-IDM_SERVER_VIEW_ITEM_BEGIN;
		CServerViewItemArray &ServerViewItemArray=m_pPlatformServerSink->GetServerViewArrary();

		//激活窗口
		ASSERT(CWndViewItemCtrl::GetInstance()!=NULL);
		CWndViewItemCtrl::GetInstance()->ActiveViewItem(ServerViewItemArray[nIndex]);
...
VOID CServerViewItem::RectifyControl(INT nWidth, INT nHeight)
{
	...
	if (m_bHideUserList==false)
	{
		if (m_bRectifyVor==false)
		{
			...
			rcVorSplitter.left=nWidth-rcMainWnd.Width()*6/25+10;
	...
	rcAD.top=0;
	rcAD.right=nWidth;
	rcAD.left=rcVorSplitter.right;
	rcAD.bottom=PUBLICIZE_CY;
	...
	DeferWindowPos(hDwp,m_btQuitServer,NULL,rcTableArea.right-rcQuitServer.Width()-4,5,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btOtherServer,NULL,20,2,0,0,uFlags|SWP_NOSIZE);
	...
	INT nXPosChat=rcChatArea.left+6;
	INT nYPosChat=rcChatArea.bottom-64;
	...
	DeferWindowPos(hDwp,m_btSendChat,NULL,rcChatArea.right-38,rcChatArea.bottom-33,0,0,uTargetFlag|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btChatShort,NULL,rcChatArea.right-61,rcChatArea.bottom-33,0,0,uTargetFlag|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_ChatEdit,NULL,rcChatArea.left+10,rcChatArea.bottom-28,rcChatArea.Width()-75,12,uTargetFlag);
	if (!m_bEnableChat)
	{
		m_btSendChat.EnableWindow(false);
		m_btChatShort.EnableWindow(false);
		...
		//m_btSendChat.ShowWindow(SW_HIDE);
		//m_btChatShort.ShowWindow(SW_HIDE);
		//m_ChatEdit.EnableWindow(SW_HIDE);
	}
	...
	m_ADEncircle.DeferWindowPos(&m_PlatformPublicize,hDwp,rcAD);
...
//配置房间
VOID CServerViewItem::InitServerViewItem(CGameServerItem * pGameServerItem,WORD wAVServerPort,DWORD dwAVServerAddr,IPlatformServerSink *pPlatformServerSink)
{
	//设置状态
	...
	//设置接口
	m_pPlatformServerSink=pPlatformServerSink;
	...
	PropertyResource.SizeExcursion.cy=38;
	...
	_sntprintf(szServerTitle,CountArray(szServerTitle),TEXT("[ %s ] - [%ld]"),m_GameServer.szServerName, m_GameServer.dwOnLineCount);
...
BOOL CServerViewItem::OnEraseBkgnd(CDC * pDC)
{
	...
	if (m_bHideUserList==false)
	{
		CRect rcAD;
		rcAD.top=0;
		rcAD.right=rcClient.Width();
		rcAD.left=rcVorSplitter.right;
		rcAD.bottom=PUBLICIZE_CY;
		m_ADEncircle.DrawEncircleFrame(pDC,rcAD);
	}
	...
	pDC->SetTextColor(RGB(251,239,202));
	pDC->SelectObject(CSkinResourceManager::GetInstance()->GetDefaultFont());

	//加载资源
	CPngImage ImageServerNameBK;
	ImageServerNameBK.LoadImage(AfxGetInstanceHandle(), TEXT("SERVER_NAME_BK"));

	//kind名字
	ImageServerNameBK.DrawImage(pDC, 110, 4);
	CRect rcKindName(110,8,195,20);
	pDC->DrawText(m_GameKind.szKindName,lstrlen(m_GameKind.szKindName),&rcKindName,DT_CENTER|DT_END_ELLIPSIS|DT_VCENTER|DT_SINGLELINE);
	...
	//绘画标题
	CRect rcTitle(212,8,rcVorSplitter.left-TABLE_BUTTON_CX-8,20);
...
//其他房间
void CServerViewItem::OnClickedOtherServer()
{
	//指针判断
	if(m_pPlatformServerSink==NULL) return;
	...
	//更多菜单
	FunctionMenu.AppendMenu(IDM_SERVER_VIEW_ITEM_NULL,TEXT("游戏广场"));

	//更多菜单
	for(int i=0; i<nItemCount; i++)
	{
		if(ServerViewItemArray[i]->GetServerID()!=GetServerID())
		{
			FunctionMenu.AppendMenu(IDM_SERVER_VIEW_ITEM_BEGIN+i,ServerViewItemArray[i]->GetServerName());
		}
	}

	//弹出菜单
	CRect rcButton;
	m_btOtherServer.GetWindowRect(&rcButton);
	FunctionMenu.TrackPopupMenu(rcButton.left,rcButton.bottom,this);
}


//是否允许聊天
bool CServerViewItem::IsEnableChat() const
{
	return m_bEnableChat;
}


//设置是否允许聊天
void CServerViewItem::EnableChat( bool bEnable )
{
	m_bEnableChat = bEnable;
}
