...
BOOL CDlgWhisper::OnInitDialog()
{
	...
	//定义变量
	TCHAR szTitle[32]=TEXT("私聊");

	//获取用户
	if (m_ClientUserItemArray.GetCount()>=2L)
	{
		IClientUserItem * pIClientUserItem=m_ClientUserItemArray[1];
		_sntprintf(szTitle,CountArray(szTitle),TEXT("与 %s 私聊"),pIClientUserItem->GetNickName());
	}
...
VOID CDlgWhisper::OnDrawClientArea(CDC * pDC, INT nWidth, INT nHeight)
{
	...
	//绘画背景
	COLORREF crBackGround=RGB(227,220,214);
	...
	pDC->FillSolidRect(rcChatMessage.right,nTBorder+USER_INFO_CY,USER_INFO_CX,5,crBackGround);
