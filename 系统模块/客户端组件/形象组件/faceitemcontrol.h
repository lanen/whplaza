...
protected:
	...
	CPngImage *					m_arrFaceLarge;					//用户大头像数组

	//头像框架
protected:
	CSize							m_SizeItemFrame;					//框架大小
	CPngImage					m_ImageItemFrame;					//头像框架
	CSize							m_SizeLargeFaceFrame;					//用户大头像框架大小
	CPngImage					m_imgLargeFaceFrame;				//用户大头像框架
...
public:
	...
	//绘画用户信息头像框架
	virtual VOID DrawFaceUserInfoFrame(CDC * pDC, INT nXPos, INT nYPos, BYTE cbFrameKind);
...
public:
	...
	//绘画用户信息大头像
	virtual VOID DrawFaceUserInfo(CDC * pDC, INT nXPos, INT nYPos, WORD wFaceID);
	//绘画头像
