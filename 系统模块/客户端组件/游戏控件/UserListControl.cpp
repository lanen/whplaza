...
VOID CUserListControl::PreSubclassWindow()
{
	...
	//InsertColumn(m_wColumnCount++,TEXT("昵称"),LVCFMT_CENTER,120,0);

	////ID
	//m_cbDataDescribe[m_wColumnCount]=UD_USER_ID;
	//InsertColumn(m_wColumnCount++,TEXT("ID"),LVCFMT_CENTER,65,0);

	////游戏币
	//m_cbDataDescribe[m_wColumnCount]=UD_SCORE;
	//InsertColumn(m_wColumnCount++,TEXT("游戏币"),LVCFMT_CENTER,65,0);
	...
	//InsertColumn(m_wColumnCount++,TEXT("级别"),LVCFMT_CENTER,90,0);

	////经验值
	//m_cbDataDescribe[m_wColumnCount]=UD_EXPERIENCE;
	//InsertColumn(m_wColumnCount++,TEXT("经验值"),LVCFMT_CENTER,120,0);

	////魅力值
	//m_cbDataDescribe[m_wColumnCount]=UD_LOVELINESS;
	//InsertColumn(m_wColumnCount++,TEXT("魅力值"),LVCFMT_CENTER,65,0);

	////奖牌
	//m_cbDataDescribe[m_wColumnCount]=UD_USER_MEDAL;
	//InsertColumn(m_wColumnCount++,TEXT("奖牌"),LVCFMT_CENTER,55,0);

	////胜率
	//m_cbDataDescribe[m_wColumnCount]=UD_WIN_RATE;
	//InsertColumn(m_wColumnCount++,TEXT("胜率"),LVCFMT_CENTER,55,0);

	////逃率
	//m_cbDataDescribe[m_wColumnCount]=UD_FLEE_RATE;
	//InsertColumn(m_wColumnCount++,TEXT("逃率"),LVCFMT_CENTER,55,0);

	////总局
	//m_cbDataDescribe[m_wColumnCount]=UD_PLAY_COUNT;
	//InsertColumn(m_wColumnCount++,TEXT("总局"),LVCFMT_CENTER,55,0);

	////赢局
	//m_cbDataDescribe[m_wColumnCount]=UD_WIN_COUNT;
	//InsertColumn(m_wColumnCount++,TEXT("赢局"),LVCFMT_CENTER,55,0);

	////输局
	//m_cbDataDescribe[m_wColumnCount]=UD_LOST_COUNT;
	//InsertColumn(m_wColumnCount++,TEXT("输局"),LVCFMT_CENTER,55,0);

	////和局
	//m_cbDataDescribe[m_wColumnCount]=UD_DRAW_COUNT;
	//InsertColumn(m_wColumnCount++,TEXT("和局"),LVCFMT_CENTER,55,0);

	////逃局
	//m_cbDataDescribe[m_wColumnCount]=UD_FLEE_COUNT;
	//InsertColumn(m_wColumnCount++,TEXT("逃局"),LVCFMT_CENTER,55,0);

	////签名
	////m_cbDataDescribe[m_wColumnCount]=UD_UNDER_WRITE;
	//InsertColumn(m_wColumnCount++,TEXT("签名"),LVCFMT_CENTER,200,0);

	////社团
	////m_cbDataDescribe[m_wColumnCount]=UD_GROUP_NAME;
	//InsertColumn(m_wColumnCount++,TEXT("社团"),LVCFMT_CENTER,120,0);
	...
	case UD_IMAGE_STATUS:	//用户状态
		{
			...
			UINT uImageIndex= ( INDEX_STATUS+cbGender*COUNT_STATUS+cbUserStatus-US_FREE ) % ( US_STATUS_NUM * 2 );
