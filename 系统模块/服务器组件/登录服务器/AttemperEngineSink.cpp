	...
	case SUB_GP_GET_LIST:			//获取列表
		{
			...
			if (m_pInitParameter->m_cbDelayList==TRUE)
			{
				...
				SendGameServerInfo(dwSocketID,INVALID_WORD, INVALID_WORD);
			}
			else
			{
				//发送页面
	...
	case SUB_GP_GET_SERVER:			//获取房间
		{
			...
			for (UINT i=0;i<nKindCount;i++)
			{
				...
				SendGameServerInfo(dwSocketID,((WORD *)pData)[i], INVALID_WORD);
			}
...
bool CAttemperEngineSink::OnDBPCLogonSuccess(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	pCMDLogonSuccess->dwAgentID=pDBOLogonSuccess->dwAgentID;
	...
	//发送列表
	if (TRUE || m_pInitParameter->m_cbDelayList==TRUE)
	{
		//发送列表
		...
		SendGameServerInfo(dwContextID,INVALID_WORD,pDBOLogonSuccess->dwAgentID);
...
//发送房间
VOID CAttemperEngineSink::SendGameServerInfo(DWORD dwSocketID, WORD wKindID, DWORD dwAgentID)
{
	//网络数据
	WORD wSendSize=0;
	BYTE cbDataBuffer[SOCKET_TCP_PACKET];
	...
	for (DWORD i=0;i<m_ServerListManager.GetGameServerCount();i++)
	{
		...
		//不同代理显示不同的房间
		INT nAgentCount = strlen(pGameServerItem->m_GameServer.szAgentGroup)/4;
		bool bSend = true;
		for(INT i = 0; i < nAgentCount; i++)
		{
			char szAgentID[4] = {0};
			szAgentID[0] = pGameServerItem->m_GameServer.szAgentGroup[0+i*4];
			szAgentID[1] = pGameServerItem->m_GameServer.szAgentGroup[1+i*4];
			szAgentID[2] = pGameServerItem->m_GameServer.szAgentGroup[2+i*4];
			szAgentID[3] = pGameServerItem->m_GameServer.szAgentGroup[3+i*4];

			INT nAgentID = atoi(szAgentID);
			if(nAgentID != 0 && nAgentID != dwAgentID)
			{
				bSend = false;
			}
			else
