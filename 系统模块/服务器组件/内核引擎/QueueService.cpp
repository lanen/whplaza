...
bool  CQueueService::StartService()
{
	...
	if (m_hCompletionPort == NULL) throw TEXT("亿网棋牌队列对象完成端口创建失败");

	//启动线程
	if (m_QueueServiceThread.InitThread(m_hCompletionPort) == false) throw TEXT("亿网棋牌队列对象线程初始化失败");
	if (m_QueueServiceThread.StartThread() == false) throw TEXT("亿网棋牌队列对象线程启动失败");
