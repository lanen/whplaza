...
VOID CGamePropertyCtrl::RectifyControl(INT nWidth, INT nHeight)
{
	...
	DeferWindowPos(hDwp,m_btLeftMove,NULL,(nTempWidth-rcButton.Width())/2+3,(nHeight-rcButton.Height())/2,0,0,uFlags|SWP_NOSIZE);
	...
	//左移按钮
	if( (m_cbPresentStartIndex+nItemCount>=m_cbPresentCount && m_cbPropertyStartIndex+nItemCount>=m_cbPropertyCount) || !m_bEnable )
		m_btRightMove.EnableWindow(FALSE);
	else
		m_btRightMove.EnableWindow(TRUE);

	//右移按钮
	if( (m_cbPresentStartIndex==0 && m_cbPropertyStartIndex==0) || !m_bEnable )
...
BOOL CGamePropertyCtrl::OnEraseBkgnd(CDC * pDC)
{
	...
	//提示
	if( m_bEnable && m_wHoverItem != INVALID_WORD && nXHoverPos != -1 && nYHoverPos != -1)
	{
		//获取信息
		TCHAR *pName=NULL;
...
VOID CGamePropertyCtrl::OnLButtonUp(UINT nFlags, CPoint MousePoint)
{
	...
	//动作处理
	if ( m_bEnable && m_wDownItem!=INVALID_WORD)
	{
		//变量定义
...
BOOL CGamePropertyCtrl::OnSetCursor(CWnd * pWnd, UINT nHitTest, UINT uMessage)
{
	...
	//设置光标
	if ( m_bEnable && m_wHoverItem!=INVALID_WORD )
	{
...
//道具是否可用
bool CGamePropertyCtrl::IsEnable() const
{
	return m_bEnable;
}
...
//设置道具是否可用
void CGamePropertyCtrl::EnableProperty( bool bEnable )
{
	m_bEnable = bEnable;
}
