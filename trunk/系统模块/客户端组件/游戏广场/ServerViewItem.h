...
//查询接口
interface IPlatformServerSink
{
	//获取房间
	virtual CServerViewItemArray& GetServerViewArrary()=NULL;
	//进入房间
	virtual void EnterServerViewItem(CServerViewItem *pServerViewItem)=NULL;
};
...
public:
	...
	//配置房间
	VOID InitServerViewItem(CGameServerItem * pGameServerItem,WORD wAVServerPort,DWORD dwAVServerAddr, IPlatformServerSink *pPlatformServerSink);
	//设置界面
...
protected:
	//其他房间
	void OnClickedOtherServer();

/*对外接口*/
	//是否允许聊天
	bool IsEnableChat() const;
	//设置是否允许聊天
...
public:
	...
	CSkinButton						m_btOtherServer;					//控制按钮
...
protected:
	...
	CEncircleBMP					m_ADEncircle;								//用户资源
