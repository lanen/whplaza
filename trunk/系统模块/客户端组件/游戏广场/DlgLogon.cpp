...
CDlgLogon::CDlgLogon() : CDialog(IDD_DLG_LOGON)
{
	...
	//创建画刷
	m_brBrush.CreateSolidBrush(RGB(87,51,0));
	m_brBrush2.CreateSolidBrush(RGB(87,51,0));
...
CDlgLogon::~CDlgLogon()
{
	...
	if(m_brBrush2.GetSafeHandle()!=NULL)
	{
		m_brBrush2.DeleteObject();
	}
...
BOOL CDlgLogon::OnInitDialog()
{
	...
	m_btEnter.SetButtonImage(IDB_BT_LOGON_ENTER,hInstance,false,false);
	m_btOption.SetButtonImage(IDB_BT_LOGON_ITEM,hInstance,false,false);
	m_btRegister.SetButtonImage(IDB_BT_LOGON,TEXT("BT_LOGON_REGISTER"),hInstance,false,false);
	m_btDeleteRecord.SetButtonImage(IDB_BT_LOGON_ITEM,hInstance,false,false);
	...
	//调整位置
	CSkinButton * pSkinButtons[]={&m_btLogonLink1,&m_btLogonLink2,&m_btLogonLink3,&m_btLogonLink4};
	for (INT i=0;i<CountArray(pSkinButtons);i++)
	{
		CRect crbtlink;
		pSkinButtons[i]->GetWindowRect(&crbtlink);
		ScreenToClient(&crbtlink);
		crbtlink.top += 1; crbtlink.bottom += 1;
		pSkinButtons[i]->SetWindowPos(NULL,crbtlink.left,crbtlink.top,crbtlink.Width(),crbtlink.Height(),SWP_NOZORDER|SWP_NOCOPYBITS|SWP_DEFERERASE);
	}
	...
	_sntprintf(szBillUrl,CountArray(szBillUrl),TEXT("%s/l2.html"),szPlatformLink);

	//设置广告
	m_PlatformPublicize.SetBoradColor(RGB(51,25,3));
	m_PlatformPublicize.Navigate(szBillUrl);
	m_PlatformPublicize.SetWindowPos(NULL,LAYERED_SIZE,37,SizeWindow.cx-2*LAYERED_SIZE,116,SWP_NOZORDER|SWP_NOCOPYBITS|SWP_NOACTIVATE);
	...
	//m_TabControl.SetTabCtrlColor(RGB(65,33,6),RGB(141,71,8));
	//m_TabControl.SetTabTextColor(RGB(236,215,192));
	...
	case CTLCOLOR_STATIC:
		{
			...
			pDC->SetTextColor(RGB(236,215,192));

			if(pWnd->GetDlgCtrlID()==IDC_REM_PASSWORD)
			{
				return m_brBrush2;
			}
