...
bool CTableResource::UpdateResource(CSkinRenderManager * pSkinRenderManager)
{
	...
	//头像
	{
		...
		if (m_ImageUserFace.LoadImage(GetModuleHandle(AVATAR_CONTROL_DLL_NAME),TEXT("USER_FACE_IMAGE"))==false) return false;
...
VOID CTableView::DrawTableView(CDC * pDC, INT nXPos, INT nYPos, DWORD dwServerRule, CTableResource * pTableResource)
{
	...
	if ((pTableResource->m_bShowEnter==true)&&(pTableResource->m_ImageEnter.IsNull()==false))
	{
		...
		pTableResource->m_ImageEnter.DrawImage(pDC,nXPos+pTableResource->m_ptEnter.x,nYPos+pTableResource->m_ptEnter.y,SizeEnter.cx,SizeEnter.cy,
	...
	if(m_TableAttribute.wChairCount < MAX_CHAIR)
	{
		for (WORD i=0;i<m_TableAttribute.wChairCount;i++)
		{
			...
			if ((bAvertCheatMode==true)&&(pTableResource->m_bShowChair==true)&&(m_TableAttribute.wTableID==0))
			{
				...
				pTableResource->m_ImageChair.DrawImage(pDC,nXPos+rcChair.left,nYPos+rcChair.top,SizeChair.cx,SizeChair.cy,SizeChair.cx*i,0);
			}
			...
			if (pTableResource->m_bShowUser==true)
			{
				...
				if ((bAvertCheatMode==false)&&(m_TableAttribute.pIClientUserItem[i]!=NULL))
				{
					...
					if (pTableResource->m_bCustomUser==false)
					{
						...
						//绘画头像
						DrawUserAvatar(pDC,nXPos+rcChair.left+nXExcursion,nYPos+rcChair.top+nYExcursion,pIClientUserItem,pTableResource);
					}
					else
					{
						//自定头像
						...
						pTableResource->m_ImageCustomUser.DrawImage(pDC,nXPos+rcChair.left,nYPos+rcChair.top,rcChair.Width(),rcChair.Height(),nXImagePos,nYImagePos);
					}
					...
					{
						INT nXPos2=pTableResource->m_ptReadyArray[i].x;
						INT nYPos2=pTableResource->m_ptReadyArray[i].y;
						pTableResource->m_ImageReady.DrawImage(pDC,nXPos+nXPos2,nYPos+nYPos2);
					}

					//用户名字
					DrawTableName(pDC,nXPos,nYPos,i,pIClientUserItem->GetNickName(),pTableResource);
				}
				...
				if ((bAvertCheatMode==true)&&(pTableResource->m_bShowUser==true))
				{
					...
					if (m_TableAttribute.wTableID>0)
					{
						...
						if (pTableResource->m_bCustomUser==false)
						{
							...
							pIFaceItemControl->DrawFaceNormal(pDC,nXPos+rcChair.left+nXExcursion,nYPos+rcChair.top+nYExcursion,((m_TableAttribute.wTableID*m_TableAttribute.wChairCount)+i)%pIFaceItemControl->GetFaceCount());
						}
						else
						{
							//自定头像
					...
					DrawTableName(pDC,nXPos,nYPos,i,(wTableID==0)?TEXT("点击加入"):TEXT("分配位置"),pTableResource);
				}
				...
				if ((bAvertCheatMode==false)||(m_TableAttribute.wTableID==0))
				{
					...
					{
						pDC->Draw3dRect(nXPos+rcChair.left,nYPos+rcChair.top,rcChair.Width(),rcChair.Height(),RGB(125,125,125),RGB(125,125,125));
					}
					...
					if ((pTableResource->m_bCustomUser==false)&&((m_wHoverChairID==i)||(m_TableAttribute.pIClientUserItem[i]!=NULL)))
					{
						...
						//绘画框架
						IFaceItemControl * pIFaceItemControl=CFaceItemControl::GetInstance();
						pIFaceItemControl->DrawFaceItemFrame(pDC,nXPos+rcChair.left+nXExcursion,nYPos+rcChair.top+nYExcursion,cbFrameKind);
					}
				}
			}
		}
	...
	{
		INT nXPos2=pTableResource->m_ptLock.x;
		INT nYPos2=pTableResource->m_ptLock.y;
		pTableResource->m_ImageLocker.DrawImage(pDC,nXPos+nXPos2,nYPos+nYPos2);
	}
	...
	if (pTableResource->m_bShowTableID==true)
	{
		...
		pTableResource->m_ImageNumberBK.DrawImage(pDC,nXPos+nXNumberBK,nYPos+nYNumberBK);
		...
		for (INT i=0;i<wItemCount;i++)
		{
			...
			pTableResource->m_ImageNumberNB.DrawImage(pDC,nXPos+nXNumberNB,nYPos+nYNumberNB,SizeNumberNB.cx,SizeNumberNB.cy,wNumber*SizeNumberNB.cx,0);
	...
	{
		pDC->Draw3dRect(nXPos,nYPos,SizeTable.cx,SizeTable.cy,RGB(125,125,125),RGB(125,125,125));
	}
...
//绘画名字
VOID CTableView::DrawTableName(CDC * pDC, INT nXPos1, INT nYPos1, WORD wChairID, LPCTSTR pszNickName, CTableResource * pTableResource)
{
	//用户名字
	CRect rcName=pTableResource->m_rcNameArray[wChairID];
	rcName.left+=nXPos1;rcName.right+=nXPos1;
	rcName.top+=nYPos1;rcName.bottom+=nYPos1;
	...
	else
	{
		...
		if(CD2DEngine::GetD2DEngine())
		{
			...
			if (bOffLine==false)
			{
				...
				pTableResource->m_ImageUserFace.DrawImage(pDC,nXPos,nYPos,nWidth,nHeight,nXImagePos*FACE_CX,nYImagePos*FACE_CY);
			}
			else
			{
				//调整参数
				...
				INT nXImagePos=wFaceID%wLineCount;
				INT nYImagePos=(wFaceID/wLineCount)*2L;
				pTableResource->m_ImageUserFace.DrawImage(pDC,nXPos,nYPos,nWidth,nHeight,nXImagePos*FACE_CX,nYImagePos*FACE_CY+FACE_CY);
			}
		}
		else
		{
...
VOID CTableViewFrame::OnPaint()
{
	...
	//if (m_TableResource.m_ImageGround.IsNull()==false)
	//{
	//	m_TableResource.m_ImageGround.BitBlt(pBufferDC->GetSafeHdc(),0,0);
	//}
	for(int iY=0; iY<rcClient.Height(); iY+=m_TableResource.m_ImageGround.GetHeight())
	{
		for(int iX=0; iX<rcClient.Width(); iX+=m_TableResource.m_ImageGround.GetWidth())
		{
			m_TableResource.m_ImageGround.DrawImageTile(pBufferDC,iX,iY,m_TableResource.m_ImageGround.GetWidth(),m_TableResource.m_ImageGround.GetHeight());
		}
	}
	...
	if ((m_TableViewArray.GetCount()>0L)&&(m_TableResource.m_bShowTable==true))
	{
		...
		if(bAvertCheatMode)
		{
			...
			if(bAvertCheatMode && m_bWaitDistribute)
			{
				...
				nXExcursion=(rcClient.Width() - SizeTable.cx)/2;
				nYExcursion=(rcClient.Height() - SizeTable.cy)/2;
			}
		}
		...
		for (INT nYIndex=nYStartItem;nYIndex<nYConcludeItem;nYIndex++)
		{
			for (INT nXIndex=nXStartItem;nXIndex<nXConcludeItem;nXIndex++)
			{
				...
				if ((nIndex>=0)&&(nXIndex<nXTableCount)&&(nIndex<wTableCount))
				{
					...
					if(bAvertCheatMode && m_bWaitDistribute)
					{
						m_TableResource.m_ImageWaitDistribute.BitBlt(pBufferDC->GetSafeHdc(),nXTablePos,nYTablePos);
					}
					else
					{
						m_TableViewArray[nIndex]->DrawTableView(pBufferDC,nXTablePos,nYTablePos,m_dwServerRule,&m_TableResource);
					}
