...
CWndUserInfoCtrl::CWndUserInfoCtrl()
{
	...
	//设置区域
	m_rcFaceArea.SetRect( 10, 5, 220, 227 );
	m_iLargeFacePosX = 0;
	m_iLargeFacePosY = 0;
...
VOID CWndUserInfoCtrl::RectifyControl(INT nWidth, INT nHeight)
{
	...
	m_iLargeFacePosX = ( nWidth - FACE_USER_INFO_CX ) / 2;
	m_iLargeFacePosY = 2;
	m_rcFaceArea.SetRect( m_iLargeFacePosX, m_iLargeFacePosY, FACE_USER_INFO_CX, FACE_USER_INFO_CY );
...
BOOL CWndUserInfoCtrl::OnEraseBkgnd(CDC * pDC)
{
	...
	//背景
	CPngImage	 UserInfoBk;
	//图片资源
	UserInfoBk.LoadImage(AfxGetInstanceHandle(),TEXT("USER_INFO_BK"));
	//绘制图片
	UserInfoBk.DrawImage(pBufferDC, 0, 0, rcClient.Width(), rcClient.Height(), 0, 0, UserInfoBk.GetWidth(), UserInfoBk.GetHeight() );
	...
	if (pGlobalUserData->dwUserID!=0L)
	{
		...
		{
			theApp.m_FaceItemControlModule->DrawFaceUserInfo(pBufferDC, m_iLargeFacePosX, m_iLargeFacePosY, pGlobalUserData->wFaceID);
		}

		//绘画边框
		if ( m_bHoverFace && !m_bClickFace )
		{
			theApp.m_FaceItemControlModule->DrawFaceUserInfoFrame(pBufferDC,m_iLargeFacePosX-5,m_iLargeFacePosY-5,ITEM_FRAME_HOVER);
		}
		else if ( m_bClickFace && m_bHoverFace ) 
		{
			theApp.m_FaceItemControlModule->DrawFaceUserInfoFrame(pBufferDC,m_iLargeFacePosX-5,m_iLargeFacePosY-5,ITEM_FRAME_HITDOWN);
		}
		else
		{
			theApp.m_FaceItemControlModule->DrawFaceUserInfoFrame(pBufferDC,m_iLargeFacePosX-5,m_iLargeFacePosY-5,ITEM_FRAME_NORMAL);
		}

		//绘画标题
		pBufferDC->SetTextColor(RGB(251,188,55));
		//pBufferDC.TextOut(15,265,TEXT("用户名："));
		//pBufferDC.TextOut(15,285,TEXT("财富值："));
		//pBufferDC.TextOut(15 ,305,TEXT("奖牌数："));
		...
		//构造财富值
		TCHAR szUseTreasure[MAX_PATH]=TEXT("");
		_sntprintf(szUseTreasure,CountArray(szUseTreasure),TEXT("%ld"),pGlobalUserData->lUserScore);

		//绘制财富
		CBitImage ImageWealth;
		ImageWealth.LoadFromResource(AfxGetInstanceHandle(),MAKEINTRESOURCE(IDB_WEALTHINFO));
		ImageWealth.BitBlt(BufferDC,10,210);

		pBufferDC->TextOut(110,220,TEXT(szUseTreasure));
		pBufferDC->TextOut(110,250,TEXT(szUserMedal));


		////输出奖牌
		//CRect rcUserMedal;
		//rcUserMedal.SetRect(65,305,rcClient.Width()-15,317);
		//pBufferDC.DrawText(szUserMedal,lstrlen(szUserMedal),&rcUserMedal,DT_SINGLELINE|DT_VCENTER|DT_END_ELLIPSIS);

		////输出财富值
		//CRect rcUserTreasure;
		//rcUserTreasure.SetRect( 65, 285, rcClient.Width()-15, 297 );
		//pBufferDC.DrawText(szUseTreasure,lstrlen(szUseTreasure),&rcUserTreasure,DT_SINGLELINE|DT_VCENTER|DT_END_ELLIPSIS);

		////输出帐号
		//CRect rcNickName;
		//rcNickName.SetRect( 65, 265, rcClient.Width()-15, 277 );
